...
function show_help() {
  ...
  Options:
  \t-i --image_name\t\t Name of the image to be run (default robot-humble-torch-arm64).
  \t-c --container_name\t Name of the container (default robot_humble_torch_container).
  \t-w --workspace\t\t Host workspace directory name (default: robot_torch_ws). 
...
IMAGE_NAME=${IMAGE_NAME:-robot-humble-torch-arm64}
CONTAINER_NAME=${CONTAINER_NAME:-robot_humble_torch_container}
...
WORKSPACE_HOST_DIR=${WORKSPACE_HOST_DIR:-robot_torch_ws}
...
docker run --privileged --net=host -it $NVIDIA_FLAGS \
       ...
       -v /home/bueche/temp/torch_lib:/usr/local/lib/python3.10/dist-packages/torch  \
       -v /dev:/dev \
       -u $HOST_UID:$HOST_GID \
